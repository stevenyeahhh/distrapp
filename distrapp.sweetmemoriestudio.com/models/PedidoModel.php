...
//        return $this->getDb()->selectQuery("PEDIDO", "*", "");
    ...
    public function getPedidosCountEntregados() {
        return Singleton::getInstance()->db->selectQuery("PEDIDO", "COUNT(*) CNT , (CASE WHEN ENTREGADO = 1 THEN 'ENTREGADO' ELSE 'NO ENTREGADO' END) descripcion" , "1 group by (CASE WHEN ENTREGADO = 1 THEN 'ENTREGADO' ELSE 'NO ENTREGADO' END)")->fetchAll(PDO::FETCH_ASSOC);
    }
    public function cambiarEstadoPedido($pedido,$estado) {
        return Singleton::getInstance()->db->updateQuery(" PEDIDO ", " ENTREGADO='$estado' ", " ID_PEDIDO ='$pedido' ")->rowCount();
    }
